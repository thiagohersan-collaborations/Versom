...
class WordShape implements AShape {
  ...
  // Constructor
  WordShape(float x, float y, String s) {
    ...
    println(myString);
    //yOff = 0;
    //if ((myString.contains("g"))||(myString.contains("j"))||(myString.contains("p"))||(myString.contains("q"))||(myString.contains("y")) ) {
    //yOff += textDescent();
    //}    
    ...
    for (int i=0; i<bodies.size(); i++) {
      ...
      if (DEBUG) {
        ...
        for (int j = 0; j < ps.getVertexCount(); j++) {
          Vec2 v = box2d.vectorWorldToPixels(ps.getVertex(j));
          vertex(v.x, v.y);
        }
        fill(175, alphaVal);
        stroke(0, alphaVal);
  ...
  void makeBody(Vec2 xy) {
    ...
    for (int i=(myString.length()-1); i >= 0; i--) {
      ...
      vertices[0] = box2d.vectorPixelsToWorld(new Vec2(-cW/2, -textHeight/2));
      vertices[1] = box2d.vectorPixelsToWorld(new Vec2(-cW/2, textHeight/2+myOff));
      vertices[2] = box2d.vectorPixelsToWorld(new Vec2(cW/2, textHeight/2+myOff));
      vertices[3] = box2d.vectorPixelsToWorld(new Vec2(cW/2, -textHeight/2));
      ...
      bd.position.set(box2d.coordPixelsToWorld(new Vec2(xy.x+i*2, xy.y)));
      cB = box2d.createBody(bd);
      ...
      if (i != (myString.length()-1)) {
        ...
        // top anchor points
        Vec2 pbAT = new Vec2(pB.getWorldCenter().x-box2d.scalarPixelsToWorld(textWidth(myString.charAt(i+1))/2), 
        pB.getWorldCenter().y-box2d.scalarPixelsToWorld(textHeight/2));
        Vec2 cbAT = new Vec2(cB.getWorldCenter().x+box2d.scalarPixelsToWorld(textWidth(myString.charAt(i+0))/2), 
        cB.getWorldCenter().y-box2d.scalarPixelsToWorld(textHeight/2));
        // bottom anchor points
        Vec2 pbAB = new Vec2(pB.getWorldCenter().x-box2d.scalarPixelsToWorld(textWidth(myString.charAt(i+1))/2), 
        pB.getWorldCenter().y+box2d.scalarPixelsToWorld(textHeight/2));
        Vec2 cbAB = new Vec2(cB.getWorldCenter().x+box2d.scalarPixelsToWorld(textWidth(myString.charAt(i+0))/2), 
        cB.getWorldCenter().y+box2d.scalarPixelsToWorld(textHeight/2));
        ...
        joints.add(box2d.createJoint(jDefT));
        joints.add(box2d.createJoint(jDefB));
      }
    }
  }
}
